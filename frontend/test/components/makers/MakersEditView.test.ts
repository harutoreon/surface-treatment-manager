...
describe('MakersEditView', () => {
  ...
  describe('有効な情報を送信した場合', (): void => {
    it('更新に成功すること', async (): Promise<void> => {
      ...
      const getValue = (selector: string): string => (wrapper.find(selector).element as unknown as HTMLInputElement).value

      expect(getValue('#maker-name')).toBe('有限会社中野銀行')
      expect(getValue('#maker-postal-code')).toBe('962-0713')
      expect(getValue('#maker-address')).toBe('東京都渋谷区神南1-2-0')
      expect(getValue('#maker-phone-number')).toBe('070-3288-2552')
      expect(getValue('#maker-fax-number')).toBe('070-2623-8399')
      expect(getValue('#maker-email')).toBe('sample_maker0@example.com')
      expect(getValue('#maker-home-page')).toBe('https://example.com/sample_maker0')
      expect(getValue('#maker-manufacturer-rep')).toBe('宮本 悠斗')
  ...
  describe('無効な情報を送信した場合', (): void => {
    it('更新に失敗すること', async (): Promise<void> => {
      ...
      expect(wrapper.find('.alert').text()).toContain('入力に不備があります。')
    })
  })
