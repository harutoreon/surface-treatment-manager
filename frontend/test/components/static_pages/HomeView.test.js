...
describe('HomeView', () => {
  ...
  describe('ユーザーがログインした場合', () => {
    ...
    it('「アプリケーションの管理」カードが表示されること', () => {
      ...
      expect(divManageSettings.find('div p.fs-5').text()).toBe('アプリケーションの管理')
      expect(divManageSettings.find('div p.card-text').text()).toBe('アプリケーションの設定やログアウトを行います。')
  ...
  describe('一般ユーザーでログインした場合', () => {
    ...
    it('「処理名で検索」カードが表示されること', () => {
      ...
      expect(divSearchName.find('div p.fs-5').text()).toBe('処理名で検索')
      expect(divSearchName.find('div p.card-text').text()).toBe('処理名を入力して表面処理を検索します。')
    ...
    it('「カテゴリーで検索」カードが表示されること', () => {
      ...
      expect(divSearchCategory.find('div p.fs-5').text()).toBe('カテゴリーで検索')
      expect(divSearchCategory.find('div p.card-text').text()).toBe('カテゴリーを選択して表面処理を検索します。')
    ...
    it('「メーカー名で検索」カードが表示されること', () => {
      ...
      expect(divSearchMaker.find('div p.fs-5').text()).toBe('メーカー名で検索')
      expect(divSearchMaker.find('div p.card-text').text()).toBe('メーカー名を入力して表面処理を検索します。')
    ...
    it('「処理一覧から検索」カードが表示されること', () => {
      ...
      expect(divSearchList.find('div p.fs-5').text()).toBe('処理一覧から検索')
      expect(divSearchList.find('div p.card-text').text()).toBe('表面処理一覧から目的の処理を検索します。')
  ...
  describe('管理者ユーザーでログインした場合', () => {
    ...
    it('「表面処理の管理」カードが表示されること', () => {
      ...
      expect(divManageSamples.find('div p.fs-5').text()).toBe('表面処理の管理')
      expect(divManageSamples.find('div p.card-text').text()).toBe('表面処理に関する情報を一括管理します。')
    ...
    it('「カテゴリーの管理」カードが表示されること', () => {
      ...
      expect(divManageCategories.find('div p.fs-5').text()).toBe('カテゴリーの管理')
      expect(divManageCategories.find('div p.card-text').text()).toBe('カテゴリーに関する情報を一括管理します。')
    ...
    it('「メーカーの管理」カードが表示されること', () => {
      ...
      expect(divManageMakers.find('div p.fs-5').text()).toBe('メーカーの管理')
      expect(divManageMakers.find('div p.card-text').text()).toBe('メーカーに関する情報を一括管理します。')
    ...
    it('「ユーザーの管理」カードが表示されること', () => {
      ...
      expect(divManageUsers.find('div p.fs-5').text()).toBe('ユーザーの管理')
      expect(divManageUsers.find('div p.card-text').text()).toBe('ユーザーに関する情報を一括管理します。')
    ...
    it('「部署の管理」カードが表示されること', () => {
      ...
      expect(divManageDepartments.find('div p.fs-5').text()).toBe('部署の管理')
      expect(divManageDepartments.find('div p.card-text').text()).toBe('部署に関する情報を一括管理します。')
    ...
    it('「コメントの管理」カードが表示されること', () => {
      ...
      expect(divManageComments.find('div p.fs-5').text()).toBe('コメントの管理')
      expect(divManageComments.find('div p.card-text').text()).toBe('コメントに関する情報を一括管理します。')
