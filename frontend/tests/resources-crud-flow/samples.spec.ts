...
test.describe('samples crud flow', () => {
  ...
  test('表面処理情報の新規登録・参照・更新・削除ができること', async ({ page }) => {
    ...
    await page.locator('#makers').selectOption('新星コーティングス')
    await page.locator('#sample-hardness').fill('鉛筆硬度でFから3H')
    await page.locator('#sample-film-thickness').fill('5')
    ...
    await expect(page.getByRole('listitem').filter({ hasText: '5μm' })).toBeVisible()
    ...
    await expect(page.locator('#sample-film-thickness')).toHaveValue('5μm')
